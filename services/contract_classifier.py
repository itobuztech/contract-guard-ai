...
class ContractClassifier:
    ...
    def _generate_reasoning(self, contract_text: str, primary_category: str, subcategory: Optional[str], keyword_scores: Dict[str, float], 
        ...
        if (semantic_match > 0.65):
            reasoning.append(f"Contract language semantically similar to {primary_category.replace('_', ' ')} agreements (similarity: {semantic_match:.2f})")
        ...
        if (legal_bert_match > 0.60):
            reasoning.append(f"Legal-BERT semantic analysis confirms {primary_category.replace('_', ' ')} classification (similarity: {legal_bert_match:.2f})"
                            )
